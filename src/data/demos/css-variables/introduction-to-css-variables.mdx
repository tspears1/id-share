...
They are also **self-referencing**, which simplifies the _debugging_ process or making larger changes to your site.
...
.card {
  ...
  border: 3px solid var(--card-color-primary);
  ...
  &:where(.card--empire) {
    --card-color-primary: hsl(219.6 100% 50.2%);
  }

  &:where(.card--jedi) {
    --card-color-primary: hsl(0.7 89.6% 57.2%);
  }
}
```
</Sandbox>