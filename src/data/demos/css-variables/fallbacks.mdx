...
### Fallbacks vs The Cascade
Fallbacks set on a parent element and then defined on a child element will **NOT** modify the parent.
Undefined fallback variables can only be defined on the establishing element or higher in the cascade.
...
<Alert title="Thoughts on naming fallback variables" variant="warning">
   At all cost you should avoid naming fallback variables with phrases like:\
   `-default`
   `-fallback`
   `-placeholde`
   `-override`
